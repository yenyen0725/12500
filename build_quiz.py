...
<div class="home-inner">
  ...
  <div class="card">
    ...
    <div style="font-size:.82rem;color:var(--text-sub);margin-bottom:4px">
      答錯時可一鍵查詢 AI 解題原因。使用 Gemini（免費）—
      <a href="https://aistudio.google.com/app/apikey" target="_blank"
        style="color:var(--primary)">免費申請 API Key</a>
    </div>
    <div class="api-key-row">
      <input class="api-key-input" id="apiKeyInput" type="password"
        placeholder="AIza…（貼上你的 Gemini API Key）">
...
function saveApiKey(){
  ...
  if(val){
    ...
    document.getElementById('apiKeyStatus').textContent='✅ 已儲存，答錯時可點「查詢 AI 解析原因」';
  } else {
    localStorage.removeItem('geminiApiKey');
    document.getElementById('apiKeyStatus').className='api-key-status empty';
    document.getElementById('apiKeyStatus').textContent='（未設定，填入後才能使用 AI 解析）';
  }
}
...
function initApiKeyUI(){
  ...
  if(geminiApiKey){
    ...
    status.textContent='（未設定，填入後可使用 AI 解析功能）';
  }
}
...
async function askAI(){
  ...
  if(!geminiApiKey){
    alert('請先在首頁「🤖 AI 即時解析設定」填入 Gemini API Key。\n\n免費申請：https://aistudio.google.com/app/apikey');
    return;
  }
  const btn=document.getElementById('aiExplainBtn');
  btn.disabled=true;
