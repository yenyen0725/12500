...
<div class="home-inner">
  ...
  <div class="card">
    <h3>🤖 AI 即時解析設定</h3>
    <div style="font-size:.82rem;color:var(--text-sub);margin-bottom:8px">
      答錯時一鍵查詢 AI 解題原因（免費・台灣可用）—
      <a href="https://console.groq.com/keys" target="_blank"
        style="color:#6366f1;font-weight:600">免費申請 Groq Key →</a>
    </div>
    <div class="api-key-row">
      <input class="api-key-input" id="groqKeyInput" type="password"
        placeholder="gsk_…（貼上你的 Groq API Key）">
  ...
  <div class="card">
    ...
    <div id="cloudStatus" class="cloud-status">☁️ 連線中…</div>
    <div class="cloud-btn-row" style="margin-bottom:8px">
      <button class="cloud-btn" onclick="syncToCloud()">☁️ 立即上傳</button>
      <button class="cloud-btn" onclick="loadFromCloud(true)">🔄 從雲端載入</button>
    </div>
    <details style="font-size:.78rem;color:var(--text-sub)">
      <summary style="cursor:pointer">進階：自訂 Apps Script 網址</summary>
      <div class="gas-url-row" style="margin-top:8px">
        <input class="gas-url-input" id="gasUrlInput" type="text"
          placeholder="留空則使用預設網址">
...
function renderQuestion(){
  ...
  if(existingResult){
    ...
    if(existingResult.image){
      ...
      else showAnalysis(q, existingResult.correct, 'answer');
      showAIButton(q);
    }
...
function saveApiKey(){
  ...
  if(g){
    ...
    status.textContent='✅ 已儲存，答錯時可點「查詢 AI 解析原因」';
  } else {
...
function initApiKeyUI(){
  ...
  if(groqApiKey){
    status.className='api-key-status ok';
    status.textContent='✅ 已設定，答錯時可使用 AI 解析';
  } else {
    status.className='api-key-status empty';
    status.textContent='（尚未設定，填入後可使用 AI 解析功能）';
  }
}
...
async function askAI(){
  const q=quizQuestions[currentIdx];
  if(!q) return;
  // 每次點按都清掉舊快取並重新查詢（讓使用者可以修正錯誤解析）
  delete aiExplanations[q.id];
...
// 預設網址寫死在程式碼，任何設備打開都自動連線；localStorage 可覆蓋（換 Script 時用）
